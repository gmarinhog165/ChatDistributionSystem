...
public class GossipRequestHandler implements Runnable {
   ...
   private void handleEagerPush(String[] parts, int senderPort, BufferedReader in, BufferedWriter out) throws IOException {
    ...
    if (!aggregator.getSeenRequests().add(requestId)) {
        sendPrune(requestId, out, senderPort);
        return;
    }

    aggregator.recordMessageOrigin(requestId, senderPort);
    ...
    if (ttl < Config.GOSSIP_TTL) {
        ...
        notifyLazyPeers(topic, username, uuid, senderPort);
    } else {
        System.out.println("Max propagation ttl reached, switching to lazy push");
        notifyAllPeersLazy(topic, username, uuid, senderPort);
    }
...
private List<SCInfo> forwardToEagerPeers(String topic, String username, int ttl, String uuid, int senderPort, String requestId) {
    ...
    for (Integer peer : aggregator.getEagerPeers()) {
...
private void notifyLazyPeers(String topic, String username, String uuid, int senderPort) {
    for (Integer peer : aggregator.getLazyPeers()) {
        if (peer.equals(senderPort + 2)) continue;

        CompletableFuture.runAsync(() -> {
            try (Socket peerSocket = new Socket("localhost", peer - 2)) {
                BufferedWriter peerOut = new BufferedWriter(new OutputStreamWriter(peerSocket.getOutputStream()));
                peerOut.write("LAZY_PUSH " + topic + " " + username + " " + uuid + "\n");
                peerOut.flush();
            } catch (IOException e) {
    ...
    private void notifyAllPeersLazy(String topic, String username, String uuid, int senderPort) {
        for (Integer peer : aggregator.getEagerPeers()) {
            if (peer.equals(senderPort + 2)) continue;
    ...
    private void handleLazyPush(String[] parts, int senderPort, BufferedReader in, BufferedWriter out) throws IOException {
        ...
        boolean isNewRequest = !aggregator.hasSeenRequest(requestId);
        ...
        if (isNewRequest) {
            ...
            out.write("GRAFT " + topic + " " + username + " " + uuid + "\n");
        } else {
    ...
    private void handleGraft(String[] parts, int senderPort, BufferedReader in, BufferedWriter out) throws IOException {
        ...
        aggregator.moveToEagerSet(senderPort + 2);
        
        // Acknowledge the GRAFT
        out.write("GRAFT_ACK " + requestId + "\n");
        out.write("END\n");
        out.flush();
        
        // We'll handle the actual message forwarding in a separate connection
    }
